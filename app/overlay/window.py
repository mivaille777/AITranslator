...
class OverlayWindow(QWidget):
    ...
    @staticmethod
    def _coerce_max_height(value: object) -> int:
        try:
            return min(2000, max(160, int(value)))
        except (TypeError, ValueError):
    ...
    def max_height(self) -> int:
        """Maximum height of the complete Overlay card."""

        return self._max_height

    @property
    def content_scroll_area(self) -> QScrollArea:
        """Return the scroll container used for long source/translation text."""
    ...
    def _set_content_maximum_width(self) -> None:
        """Keep the complete card, including outer margins, within max_width."""

        if not hasattr(self, "_layout"):
            content_maximum_width = self._max_width
        else:
            margins = self._layout.contentsMargins()
            content_maximum_width = max(
                1,
                self._max_width - margins.left() - margins.right(),
            )
        self._source_label.setMaximumWidth(content_maximum_width)
        self._label.setMaximumWidth(content_maximum_width)

    def _set_window_size_limits(self) -> None:
        """Keep the complete card within its width and height bounds."""
        ...
        self.setMaximumSize(QSize(self._max_width, self._max_height))

    def _update_scroll_area_limits(self) -> None:
        """Clear stale scroll-area caps before measuring a new result."""

        # The custom size hint follows the content; the top-level maximum
        # height supplies the cap and turns the overflow into a scrollbar.
        self._content_scroll.setMinimumSize(QSize(0, 0))
    ...
    def _apply_content_style(self, palette: dict[str, str]) -> None:
        ...
        source_background = self._rgba_with_opacity(
            palette["label_background"],
            self._background_opacity * fade * 0.86,
            multiply_existing_alpha=True,
        )
        source_border = self._rgba_with_opacity(
            palette["border"],
            self._background_opacity * fade * 0.78,
        )
    ...
    def _apply_scroll_style(self, palette: dict[str, str]) -> None:
        """Style the optional content scrollbar as a quiet, narrow affordance."""

        handle = self._rgba_with_opacity(
            palette["muted_text"],
            self._background_opacity * 0.55,
        )
        handle_hover = self._rgba_with_opacity(
            palette["accent"],
            min(1.0, self._background_opacity * 0.9),
        )
        self._content_scroll.setStyleSheet(
            f"""
            QScrollArea#OverlayContentScroll {{
                background-color: transparent;
                border: none;
            }}
            QScrollArea#OverlayContentScroll QWidget#qt_scrollarea_viewport {{
                background-color: transparent;
                border: none;
            }}
            QScrollBar:vertical {{
    ...
    def _apply_theme(self, theme: str) -> None:
        ...
        self._apply_scroll_style(palette)
    ...
    def _resize_to_content(
        ...
        *,
        ...
        # Recalculate the parent first so both word-wrapped labels receive the
        # same available width.  Calling QLabel.adjustSize() before that
        # parent pass uses each label's independent size hint and is the
        # source of the post-dialog narrow/overlapping original row.
        self._source_label.updateGeometry()
        ...
        self.adjustSize()
        # ``adjustSize`` can change the width of the top-level card.  Activate
        # once more so height-for-width is recomputed using that final width.
        self._content_layout.invalidate()
        self._layout.invalidate()
        self._content_layout.activate()
        self._layout.activate()
        ...
        target_size = target_size.boundedTo(self.maximumSize())
        if (
            animate
            and self.isVisible()
    ...
    def refresh_layout(self) -> None:
        """Reflow both text rows after an external modal/layout operation."""
    ...
    def eventFilter(self, watched, event) -> bool:  # noqa: N802 - Qt override name
        """Forward drag/menu mouse events while leaving wheel events to Qt."""
        ...
        if watched is self._header:
            ...
            if event_type == QEvent.Type.MouseButtonRelease:
                ...
                return True
        if (
            hasattr(self, "_content_scroll")
            and watched is self._content_scroll.viewport()
        ):
            event_type = event.type()
            if event_type == QEvent.Type.MouseButtonPress:
                self.mousePressEvent(event)
