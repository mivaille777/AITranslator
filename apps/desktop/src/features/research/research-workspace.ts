...
export function filterResearchNotes(
  ...
  const sectionHeading = filters.sectionHeading?.trim().toLocaleLowerCase() ?? ""
  return notes.filter((note) => {
    ...
    if (
      sectionHeading
      && (note.section_heading.trim() || "Unsectioned evidence").toLocaleLowerCase() !== sectionHeading
    ) {
      return false
    }
...
export function researchSourceFamilies(sources: ResearchSourceSummary[]): ResearchSourceFamily[] {
  return [...new Set(sources.map((source) => source.source_family))].sort()
}
