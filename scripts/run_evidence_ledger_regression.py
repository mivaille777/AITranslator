...
DEFAULT_DATASET = "backend/evaluation/datasets/stage19_evidence_ledger.jsonl"
DEFAULT_REPORT = "test-results/evidence-ledger-regression.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the deterministic Stage 19 Evidence Ledger lifecycle benchmark. "
            "The report stores status counts and case IDs only; source text, claim "
            "statements, evidence excerpts and document URLs are excluded."
        )
    )
...
def main() -> int:
    args = parse_args()
    batch = run_stage19_evidence_ledger_benchmark(args.dataset)
    ...
    report = {
        "passed": passed,
        "protocol": "stage19.evidence-ledger@1.0.0",
        ...
        "privacy": {
            ...
            "claim_statements_in_report": False,
            "raw_evidence_text_in_report": False,
            "document_urls_in_report": False,
            "provenance_ids_in_report": False,
        },
        "batch": asdict(batch),
    }
